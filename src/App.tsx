...
// Predefined selected model URL (defaulting to the exquisite Luxury Yacht .glb)
  ...
  const handleCustomModelSelected = (file: File) => {
    setCustomModelFile(file);
    setIsUploaderOpen(false);
  };

  const handleModelChange = (url: string | null) => {
    // Revert custom uploaded file to enable seamless native preset viewing
    setCustomModelFile(null);
    setCustomModelStatus({ loaded: false, name: "" });
    setSelectedModelUrl(url);
  };

  const handleModelLoadedStatus = (success: boolean, name?: string) => {
    if (success && name) {
      setCustomModelStatus({ loaded: true, name });
    } else {
      setCustomModelStatus({ loaded: false, name: "" });
    }
  };
  ...
  return (
    <div className="relative w-full h-screen bg-[#050505] text-stone-100 overflow-hidden font-sans antialiased">
        ...
        {currentSectionIndex !== 6 && (
          ...
          >
            <ThreeCanvas
              ...
              onModelLoadedStatus={handleModelLoadedStatus}
              hullColor={hullColor}
              deckColor={deckColor}
        ...
        {currentSectionIndex !== 6 && (
          ...
          >
            <SpecificationOverlay
              ...
              isCustomLoaded={customModelStatus.loaded}
              uploadedFileName={customModelStatus.name}
              hullColor={hullColor}
      ...
      {/* MODEL UPLOAD MODAL DIALOG */}
      <AnimatePresence>
        {isUploaderOpen && (
          <ModelUploader
            isOpen={isUploaderOpen}
            onClose={() => setIsUploaderOpen(false)}
            onFileSelected={handleCustomModelSelected}
            isCustomLoaded={customModelStatus.loaded}
            uploadedFileName={customModelStatus.name}
          />
        )}
